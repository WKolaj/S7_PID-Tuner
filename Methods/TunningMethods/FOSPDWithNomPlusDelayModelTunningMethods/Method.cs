...
{
    [Method("Methods for fifth order system with nominator plus time delay model", "FOSPDWithNom.png")]
    public class Method : TuningMethod
    {
        ...
        {
            if (TuningMethod.LengthOf(TransferFunction.Nominator) != 6 || TuningMethod.LengthOf(TransferFunction.Denominator) != 6 || TransferFunction.TimeDelay <= 0)
            {
                return false;
            }
            else
