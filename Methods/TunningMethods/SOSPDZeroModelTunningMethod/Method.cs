...
using SOSPDZeroModelTunningMethod;
using System;
...
{
    [Method("Methods for second order system with zero plus time delay model", "SOSPDZero.png")]
    public class Method : TuningMethod
    {
        ...
        {
            if (TuningMethod.LengthOf(TransferFunction.Nominator) != 2 || TuningMethod.LengthOf(TransferFunction.Denominator) != 3 || TransferFunction.TimeDelay <= 0)
            {
                return false;
            }
            else
