...
{
    [Method("First order lag pulse time delay model tuning", "FOLPD.png")]
    public class Method : TuningMethod
    {
        ...
        {
            if (TuningMethod.LengthOf(TransferFunction.Nominator) != 1 || TuningMethod.LengthOf(TransferFunction.Denominator) != 2 || TransferFunction.TimeDelay <= 0)
            {
                return false;
            }
